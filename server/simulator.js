/**
 * 1000 玩家模拟器：模拟 N 个玩家跑一年，策略：学习优先 / 社交优先 / 混合 / 随机
 * 输出：atar 分布、结局分布、平均 stress、强制事件触发次数
 */
...
function runOnePlayer(seed, strategyKey) {
  ...
  const state = {
    atar: 50, mood: 70, health: 80, stress: 20, fatigue: 0,
    logic: 50, social: 50, stamina: 50
  };
  let forcedCount = 0;
...
function runSimulation(params) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    results.push(out);
    totalStress += out.stress;
    totalForced += out.forcedCount;
    const atar = out.atar;
    if (atar <= 30) atarBuckets['0-30']++;
    else if (atar <= 50) atarBuckets['31-50']++;
    else if (atar <= 70) atarBuckets['51-70']++;
    else if (atar <= 85) atarBuckets['71-85']++;
  ...
  return {
    ...
    strategyMix,
    seed: seedBase,
    atar_distribution: atarBuckets,
    ending_distribution: endingCounts,
    avg_stress: totalStress / n,
    total_forced_events: totalForced,
    avg_forced_per_player: totalForced / n
  };
}
...
function saveRun(params, result) {
  const db = getDb();
  db.prepare(
    'INSERT INTO simulation_runs (params_json, result_json) VALUES (?, ?)'
  ).run(JSON.stringify(params), JSON.stringify(result));
}

module.exports = { runSimulation, saveRun };
